...
class MFIVPipeline:
    """
    One class that replicates BOTH cells exactly:
      1) builds df11/df12 via cubic-spline strike filling per (hour, maturity, cp)
      2) builds MFIV_byH via nearest-maturity-to-H per strike, MFIV integral, and ATM BSIV
    """
    ...
    def __init__(self, df: pd.DataFrame, d_btc: pd.DataFrame, q: float = 0.0):
        ...
        # match your code: clean time index for d_btc
    ...
    def build_surfaces(self):
        ...
        self.df11, self.df12 = df11, df12
        return df11, df12

    # =========================
    # Cell 2 helpers (BS + MFIV)
    # =========================
    @staticmethod
    def _norm_cdf(x):
    ...
    def compute_mfiv_byH(self, H_LIST):
        ...
        for H in H_LIST:
            ...
            # combine calls + puts (NO maturity interpolation)  <-- same place as your code
            CP = pd.merge(
                self.df11, self.df12,
            ...
            for hour, g in tqdm(CP.groupby("hour"), total=CP["hour"].nunique()):
                ...
                # nearest maturity to H  <-- same line logic
                ...
                # ATM BSIV from nearest-maturity price  <-- same
        ...
        self.MFIV_byH = MFIV_byH
        return MFIV_byH

    def run(self, H_LIST):
        df11, df12 = self.build_surfaces()
        MFIV_byH = self.compute_mfiv_byH(H_LIST)
